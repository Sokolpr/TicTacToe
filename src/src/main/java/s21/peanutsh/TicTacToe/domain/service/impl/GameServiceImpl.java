...
public class GameServiceImpl implements GameService {
    ...
    public void isValidityGame(UUID uuidGame, UUID uuidUser, WebModel webModel) throws Exception {
        ...
        log.info(model.getStateGame().toString());
    ...
    public List<WinModel> getBestPlayers(Integer count) throws Exception {
        ...
        for (var i: listUser){
            log.info(i.toString());
            log.info(String.valueOf(modelRepository.getPercentWinningForUser(i)));
        }

        return listUser.stream()
                .map(el -> new WinModel(el, modelRepository.getPercentWinningForUser(el)))
                .sorted()
                .limit(count)
                .toList();
